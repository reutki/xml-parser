...
It does not require any installation. All code is written in vanilla JavaScript without using third-party libraries.
...
* **Do not use the same name for variables** even if they are equal in original file. For now vars can not store more than one value. For example: `<xsl name="|var1|" id="|var1|"/>` replace with `var1` and `var2`.
